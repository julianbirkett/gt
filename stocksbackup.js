...
var stockConstants = {capital: 7000, baseATR: 0.23, rsiX: 7, divergX: 9, riskATR: 2.0, rewardATR: 1.55, dateRange: 30};
var globalVars = {latestDate: [], useTable: true};
        ...
        while (true) {
            ...
            if (tickerPos >= 0) { 
                ...
                if (globalVars.latestDate[tickerPos] === "" || globalVars.latestDate[tickerPos] === undefined || checkDate > latestDate) {
                    globalVars.latestDate[tickerPos] = checkDateString;
                }
            }
...
function refreshLatestDate() {
    ...
    for (datePos = 0; datePos < globalVars.latestDate.length; datePos++) {
        ...
        checkDate = new Date(globalVars.latestDate[datePos]);
        if (latestDate === "" || checkDate > newLatestDate) {
            latestDate = globalVars.latestDate[datePos];
            newLatestDate = new Date(globalVars.latestDate[datePos]);
        }
    }
    ...
    for (datePos = 0; datePos < globalVars.latestDate.length; datePos++) {
        ...
        if (latestDate === globalVars.latestDate[datePos]) {
            latestDateCount++;
        }
    }   
    ...
    if (latestDate !== "") {
        ...
        // newLatestDate += "<div class=\"panel-body\">" + "Latest date - " + globalVars.latestDate + "</div>";
        latestDateText += "<div class=\"latestdate\" id=\"latestdate\"";
        latestDateText += "<h6>Latest date - " + latestDate + " " + "<span class=\"tag tag-default\">" + Number(latestDateCount + 1) + "/" + Number(globalVars.latestDate.length + 1)  + "</span>";
        latestDateText += "</h6>";
        latestDateText += "</div>";
        ...
        if (globalVars.useTable && !checkValue) {
           newGrid += "<tr>";
            ...
            if (checkValue) {
                ...
                newGrid += "<div class=\"col-xs-6\" style=" + "\"" + style + "\"" + ">" + size + "</div>";
                newGrid += "</div>";
...
var tickers = [{ticker: "III", desc: "3I"}, 
...
function buildScreen() {
    ...
    screen += "</p>";
    screen += "<div class=\"latestdate\" id=\"latestdate\"></div>"; // <!-- /latestdate -->
    ...
    screen += "</h1>";
    screen += "</div>";
    screen += "<p>Geordie's Patented Dodgy Stock Market Predictions</p>";            
    screen += "</div>";
